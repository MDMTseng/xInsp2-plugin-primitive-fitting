//
// algo_caliper_ransac_variants.cpp — A/B/C/D RANSAC speed/quality variants.
//
// All four share the caliper-front-end + adaptive-degree polynomial RANSAC
// core; they differ only in the *inside* of the per-degree iteration loop
// and the IRLS loss:
//
//   A — Baseline (mirror of algo_caliper_ransac.cpp).
//   B — A + cluster early termination. Each degree tracks the top-3 best
//       (coeffs, score) seen so far. Every 5 iters we sample each of the
//       three polynomials at 21 u-points and check the max pairwise L2
//       distance. If all three top hypotheses agree to <0.3 px we stop
//       the iteration loop early. Keeps quality, cuts iterations on
//       "easy" scenes by 3-5x.
//   C — A + SPRT inner rejection. Inlier counting is batched (size 8);
//       after each batch we estimate whether the hypothesis can plausibly
//       beat the best-so-far score and abort the per-hit loop early if
//       not. Iter count unchanged, but per-iter cost drops several-fold
//       on bad hypotheses.
//   D — B + C + Tukey biweight replacing Huber in the IRLS refinement.
//       Tukey is redescending (w(r) = (1 - (r/c)^2)^2 for |r|<c else 0)
//       so marginal-outlier residuals are zeroed rather than capped.
//       Quality improvement; speed unchanged.
//
// Each variant exposes a top-level entry point (detect_caliper_ransac_A,
// _B, _C, _D) so the lab benchmark can A/B them on identical scenes.
//
// Original baseline reference: algo_caliper_ransac.cpp.
//
//
...
// Variant flags. The four lab entry points instantiate four
// combinations; see file header.
enum class IRLSLoss { Huber, Tukey };
struct VariantOpts {
    bool      cluster_stop = false;  // B: top-3 curve agreement break
    bool      sprt         = false;  // C: SPRT inner rejection
    IRLSLoss  loss         = IRLSLoss::Huber; // D: Tukey biweight
};

// Sample each of N polynomials at S u-points and return the maximum
// pairwise L2 distance over the [-1,1] sample grid. Used by variant B.
double max_pairwise_curve_l2(const std::vector<std::vector<double>>& polys) {
    constexpr int S = 21;
    if (polys.size() < 2) return 0.0;
    std::vector<std::vector<double>> ys(polys.size(), std::vector<double>(S));
    for (size_t i = 0; i < polys.size(); ++i) {
        for (int s = 0; s < S; ++s) {
            double u = -1.0 + (2.0 * s) / (S - 1);
            ys[i][s] = poly_eval(polys[i], u);
        }
    }
    double max_l2 = 0.0;
    for (size_t i = 0; i < polys.size(); ++i) {
        for (size_t j = i + 1; j < polys.size(); ++j) {
            double sum_sq = 0;
            for (int s = 0; s < S; ++s) {
                double d = ys[i][s] - ys[j][s];
                sum_sq += d * d;
            }
            max_l2 = std::max(max_l2, std::sqrt(sum_sq / S));
        }
    }
    return max_l2;
}

std::vector<cv::Point2d> detect_with(const cv::Mat& gray,
                                     const GroundTruth& gt,
                                     VariantOpts opts);

} // anonymous namespace

std::vector<cv::Point2d> detect_caliper_ransac_A(const cv::Mat& gray,
                                                 const GroundTruth& gt) {
    return detect_with(gray, gt, {});
}
std::vector<cv::Point2d> detect_caliper_ransac_B(const cv::Mat& gray,
                                                 const GroundTruth& gt) {
    VariantOpts o; o.cluster_stop = true;
    return detect_with(gray, gt, o);
}
std::vector<cv::Point2d> detect_caliper_ransac_C(const cv::Mat& gray,
                                                 const GroundTruth& gt) {
    VariantOpts o; o.sprt = true;
    return detect_with(gray, gt, o);
}
std::vector<cv::Point2d> detect_caliper_ransac_D(const cv::Mat& gray,
                                                 const GroundTruth& gt) {
    VariantOpts o;
    o.cluster_stop = true; o.sprt = true; o.loss = IRLSLoss::Tukey;
    return detect_with(gray, gt, o);
}
                                     ...
                                     VariantOpts opts)
{
    ...
    for (int dsign : sign_list) {
        ...
        // Variant B: rolling top-3 (score, coeffs) for cluster-stop
        // convergence checks every CLUSTER_CHECK_EVERY iters.
        struct TopEntry { double score; std::vector<double> coeffs; };
        constexpr int    CLUSTER_TOPK         = 3;
        constexpr int    CLUSTER_CHECK_EVERY  = 5;
        constexpr double CLUSTER_TAU_PX       = 0.3;
        std::array<TopEntry, CLUSTER_TOPK> topk{};
        for (auto& e : topk) e.score = -1.0;
        ...
        for (int iter = 0; iter < ransac_iters; ++iter) {
            ...
            //
            // Variant C (SPRT inner reject): when score-so-far cannot
            // possibly beat deg_best_score even if every remaining hit
            // is an inlier in a fresh caliper, abort the sum early.
            // The tightest upper bound on (cnt × cov_frac) is
            //   (cnt + remaining) * (cov_so_far + remaining) / N_CAL
            // — generous (assumes every remaining adds a new caliper)
            // but cheap to evaluate.
            int cnt = 0;
            std::vector<int> in_idx;
            in_idx.reserve(N);
            std::vector<uint8_t> cal_hit(N_CAL, 0);
            int cal_covered_partial = 0;
            constexpr int SPRT_BATCH = 8;
            bool sprt_killed = false;
            for (int j = 0; j < N; ++j) {
                ...
                if (std::abs(r) <= FIT_THR) {
                    ...
                    if (cid >= 0 && cid < N_CAL && !cal_hit[cid]) {
                        cal_hit[cid] = 1;
                        ++cal_covered_partial;
                    }
                }
                // SPRT bound: every (j+1) % BATCH check.
                if (opts.sprt && (j + 1) % SPRT_BATCH == 0
                              && deg_best_score > 0
                              && j + 1 < N) {
                    int rem = N - (j + 1);
                    double max_cov = std::min((double)N_CAL,
                        (double)cal_covered_partial + rem) / std::max(1, N_CAL);
                    double max_score = (double)(cnt + rem) * max_cov;
                    if (max_score < deg_best_score) {
                        sprt_killed = true;
                        break;
                    }
                }
            }
            if (sprt_killed) continue;
            ...
            double cov_frac = (double)cal_covered_partial / std::max(1, N_CAL);
            ...
            // Variant B: maintain top-K by score; check cluster-agreement
            // every CLUSTER_CHECK_EVERY iters and break on convergence.
            if (opts.cluster_stop) {
                for (int t = 0; t < CLUSTER_TOPK; ++t) {
                    if (score > topk[t].score) {
                        for (int s = CLUSTER_TOPK - 1; s > t; --s)
                            topk[s] = std::move(topk[s - 1]);
                        topk[t] = {score, coeffs};
                        break;
                    }
                }
                if ((iter + 1) % CLUSTER_CHECK_EVERY == 0
                    && topk[CLUSTER_TOPK - 1].score > 0) {
                    std::vector<std::vector<double>> polys;
                    polys.reserve(CLUSTER_TOPK);
                    for (int t = 0; t < CLUSTER_TOPK; ++t) polys.push_back(topk[t].coeffs);
                    if (max_pairwise_curve_l2(polys) < CLUSTER_TAU_PX) break;
                }
            }
        }
        ...
        //         current fit's residuals, then run weighted LSQ once.
        {
            ...
            // Huber: linear-tail (|r|>c → c/|r|). Tukey: redescending
            // ((1-(r/c)^2)^2 for |r|<c else 0). Tukey's c is set to
            // FIT_THR (2.0 px) so weights drop to 0 at the inlier
            // threshold itself, while Huber's classical 0.7 keeps a
            // gentle taper.
            constexpr double C_HUBER = 0.7;
            constexpr double C_TUKEY = FIT_THR;
            for (int outer = 0; outer < 2; ++outer) {
                ...
                for (int i : best_inlier_idx) {
                    ...
                    double ar = std::abs(r);
                    double w;
                    if (opts.loss == IRLSLoss::Tukey) {
                        if (ar >= C_TUKEY) w = 0.0;
                        else { double t = ar / C_TUKEY; double u = 1.0 - t * t; w = u * u; }
                    } else {
                        w = (ar <= C_HUBER) ? 1.0 : C_HUBER / std::max(ar, 1e-9);
                    }
                    wv.push_back(w);
                }
